...
void heap_expand(uint32_t additional_size) {
  ...
  log_warning(MODULE, "OK: Heap expanded to %d KB",
              (heap_current_end - (uint32_t)heap_start) / 1024);
}
...
void *mem_alloc(size_t size) {
  ...
  while (current) {
    if (current->is_free && current->size >= formed_size) {
      log_debug(MODULE,
                "FOUND: Found free block at %x (Size: %d, Requested: %d)",
                current, current->size, formed_size);
      if (current->size > formed_size + sizeof(heap_block_t) + 4) {
        ...
        log_debug(MODULE,
                  "OK: Block at %x split. New free block at %x (Size: %d)",
                  current, new_block, new_block->size);
...
void mem_free(void *ptr) {
  ...
  if (block->magic != HEAP_MAGIC) {
    log_error(
        MODULE,
        "FAILED: KFREE Attempted to free invalid or corrupted memory at %x",
        ptr);
    return;
  }
  ...
  block->is_free = 1;
  log_debug(MODULE, "FOUND: Block at %x (Size: %d) marked free", block,
            block->size);

  if (block->next && block->next->is_free) {
    log_debug(MODULE, "OK: MERGE (Forward): %x swallowing %x", block->prev,
              block);
  ...
  if (block->prev && block->prev->is_free) {
    log_debug(MODULE, "OK: MERGE (Backward): %x swallowing %x", block->prev,
              block);
