...
void division_error_handler(uint32_t eip) {
    ...
    if (pd[pd_index] & PAGE_PRESENT) {
        ...
        if (pte & PAGE_PRESENT) {
            ...
            log_error("CPU", "Bytes at EIP: %x %x %x %x %x %x %x %x",
...
void idt_initialize(void) {
    ...
    set_idt_gate(EXCEPTION_DIV_BY_ZERO, (uint32_t)division_error_isr_wrapper, KERNEL_CS, 0x8E); // DIV/0

    uint32_t base = (idt[0].base_high << 16) | idt[0].base_low;
    log_debug(MODULE, "IDT[0] base = 0x%x (should match wrapper)", base);
    log_debug(MODULE, "division_error_isr_wrapper = 0x%x", (uint32_t)division_error_isr_wrapper);
...
bool page_fault_handler_with_error(uint32_t error_code, uint32_t eip) {
    ...
    log_error(MODULE, "PAGE FAULT at %x, error code: %x, EIP: %x", faulting_addr, error_code, eip);
...
void default_exception_handler(uint32_t eip) {
    ...
    if (pd[pd_index] & PAGE_PRESENT) {
        ...
        if (pte & PAGE_PRESENT) {
            uint32_t phys = (pte & ~0xFFF) + (eip & 0xFFF);
            uint8_t* bytes = (uint8_t*)phys;
            log_error(MODULE, "Bytes at EIP: %x %x %x %x %x %x %x %x",
                      bytes[0], bytes[1], bytes[2], bytes[3],
                      bytes[4], bytes[5], bytes[6], bytes[7]);
        }
    }
...
void debug_syscall_frame(uint32_t cs, uint32_t ss, uint32_t eip, uint32_t esp) {
    log_debug("SYSCALL", "Return frame: CS=0x%x SS=0x%x EIP=0x%x ESP=0x%x",
              cs, ss, eip, esp);
}