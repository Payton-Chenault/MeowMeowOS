...
static inline void spinlock_restore_flags(uint32_t flags) {
    // If bit 9 (IF - Interrupt Flag) was set in the saved flags, re-enable interrupts
    if (flags & (1 << 9)) {
...
static inline void spinlock_release_irq_restore(spinlock_t* lock, uint32_t flags) {
